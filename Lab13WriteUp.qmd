...
We need to read the excel file and get the sheet named 'Fartuer_vals' and 'Closer_vals'. Stored the data and combine it into a new data frame, and use `mutate()` to add the column of difference.
...
The mean and median are both above zero, which suggests the dopamine in the brains may decreases when they sing further away.
...
ggplot(ggdat) +
  ...
  stat_density(data=tibble(t=ts), aes(x=t), color="grey", geom="line")+
  geom_hline(yintercept=0) +
  geom_ribbon(data=ggdat |> filter(t <= t.critical2),
              aes(x=t, ymin=0, ymax= pdf.h0, fill="Rejection Region"), alpha=0.3) +
  geom_ribbon(data=ggdat |> filter(t >= t.critical1),
              aes(x=t, ymin=0, ymax= pdf.h0, fill="Rejection Region"), alpha=0.3) +
  geom_ribbon(data=ggdat |> filter(t >= t.stat | t <= -t.stat),
              aes(x=t, ymin=0, ymax= pdf.h0, fill="P value"), alpha=0.3) +
  # Plot observation
  geom_point(data=tibble(x=t.stat, y=0),
             aes(x=x, y=y), color="red", size=2) +
